    ...
    switch (tok[0]) {
        ...
        case 'b': {
            ...
            if (!is_admin) {
                ...
                printf("Erreur : Transaction B réservée aux non-ADMIN.\n");
            }
            break;
        }
        ...
        case 'l': {
            ...
            if (is_admin) {
                ...
                printVMsInRange(nstart, nend); // Affiche les VMs dans la plage spécifiée
            } else {
...
void printVMsInRange(int start, int end) {
    pthread_rwlock_rdlock(&rwlock_liste); // Verrou de lecture pour lister les VMs
    printf("noVM\tBusy\tTID\tDebutVM\t\tDebutCode\tFinCode\n");
    printf("==========================================================\n");
    ...
    struct noeudVM* actuel = head;
    while (actuel != NULL) {
        if (actuel->VM.noVM >= start && actuel->VM.noVM <= end) {
            printf("%d\t%d\t%d\t%p\t%p\t%p\n",
                   actuel->VM.noVM,
                   actuel->VM.busy,
                   actuel->VM.tid,
